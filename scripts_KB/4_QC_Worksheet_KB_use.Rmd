...
#eFil_long$condition <- c(rep(10,"fetal"), rep(70, "normal"), rep(423016, "tumour"))
...
#following line won't run unless i run the line above
eFil_long <- data2 %>% 
  pivot_longer(cols = -c(miRNA), names_to = "Sample", values_to = "Reads")

# eFil_long_2 <- eFil_long %>% 
#   mutate(Condition = eDat_long$Condition[match(Sample, eDat_long$Sample)])
# forgot I could left join by pDat
...
d1 <- eFil_long_2 %>% 
  ...
  # scale_colour_manual(values = colpal$Condition) +
  labs(title = "KIRC: Filtered Expression Data", y = "log2(Reads + 1)") +
  theme_minimal() 
...
#look into better ways to separate the data - not by sex, and modify later when reviewing script - you likely assigned the wrong dataframes to each plot that Nikita intended for you to make - check LIHC script plots for reference

b3 <-  pDat_c1_fetal %>% 
  ggplot(aes(x = Condition, y = Sex, fill = Condition, colour = Condition)) +
  geom_boxplot() +
  scale_fill_manual(values = colpal$Condition) +
  scale_colour_manual(values = colpal$Condition) +
  theme_minimal()
...
eNorm <- as.data.frame(row.names(eDat))
norm <- DGEList(counts = eDat, samples = pDat$Sample, genes = eNorm)
dge <- calcNormFactors(norm, method = "RLE")
rle <- cpm(dge)
...
#NEXT: try using data2 instead of eFilc1_fetal. separate out fetal samples from data2 bc i think you made the eFilc1 fetal wrong 
...
##****** what numeric value is counts supposed to be? in eFil_c1_fetal, there are 16 non-numeric variables . to keep moving on for now i will set counts = eFil_c1_fetal to the reads column - nowreads = reads_cluster_1,
